...
# THIS VERSION PRINTS BLOCKS BUT NO CHANGE INDEX: obsposs species with no index
# CURRENTLY ASSUMING FILES ARE COMING IN UPPER.CASE
...
names(block_in)[names(block_in) == 'BLOCK_ID'] <- 'block_id'

# read in the atlas data; each atlas project will be a list within a list
sp <- list(i = read.delim("WBBA1_OBSPOSSSPECIES_C3C4ONLY_FOR_CHANGEMAP.txt", quote = ""),
           ii = read.delim("WBBA2_OBSPOSSSPECIES_C3C4ONLY_FOR_CHANGEMAP.txt", quote = "")) %>%
...
names(fair)[names(fair) == 'BLOCK_ID'] <- 'BLOCK.ID'
...
filtered_sp <- read.csv("filtered_out_species_wibba20230913.csv")

# this run is on species that are obsposs with no index 
sp <- map(sp, subset, COMMON.NAME %in% 
            filtered_sp[which(filtered_sp[, "REASON"] == 
                                "obsposs and no index"), 
                        "COMMON.NAME"])
...
sp_cast <- sp %>%
  ...
  distinct(BLOCK.ID, SPEC, BREEDING, project) %>%
  # 'cast' each list
  group_by(project) %>%
...
if(anyNA(sp_cast$BLOCK.ID)) {
  warning(paste(sum(is.na(sp_cast$BLOCK.ID)), "unmatched blocks"))
}

# order the columns and rows
cols_to_keep <- c("BLOCK.ID", "BLOCK_NAME", "BLOCK_STAT")
...
if(any(duplicated(sp_cast$BLOCK.ID) | duplicated(sp_cast$BLOCK_NAME))) {
...
write.csv(change_values, here("data", "wbbaii_change-values_091123.csv"),
          row.names = FALSE)
