...
for(j in 1:length(species)){
  ...
  plot_each<-ggplot() +
    ...
    scale_color_manual(#name=paste0(" birds/km",expression("^2")),
                    ...
                    values=c("#fbcd54","#d2985c","#c18953","#a85f3b","#651312"),
                   na.translate = F)
      #guides(fill = guide_legend(label.position = "bottom",title.position = "right"))
    ## prediction and block overlay map
   # overlay_map<-plot_each+
    #  ggnewscale::new_scale_fill()+
     # geom_sf(data=plot_block_df, aes(fill=pa),color=NA,alpha=0.7) + 
      #geom_sf(data=counties, fill=NA, color="grey50", size=0.25) +
      #geom_sf(data=lakes, fill="#BFE1F4", color=NA) +
    #  theme_map() +
    ...
    #  theme(legend.title = element_text(family="mono",face="bold",size=11),
     #       legend.text = element_text(family="mono",face="bold",size=8),
      #      legend.spacing.x = unit(0, 'cm'),
       #     legend.key.width=unit(0.9, "cm"),
        #    legend.key.height =unit(0.5, "cm"))+
      #scale_fill_manual(name="Number of observations\nat atlas blocks",
       #                 labels=c("1~2",">2"),
        #                values=c("#5ec68f","#3665c7"),
         #               na.translate = F,guide = guide_legend())+
      #guides(fill = guide_legend(label.position = "bottom",title.position = "right"))
...
for(l in 1:length(sp)){
  ## make ggarrange 
  path<-"C:\\Users\\anichn\\Desktop\\2025_WBBA_finals-20250312T143626Z-001"
  
  ggsave(filename=paste0(l,"_",sp[l],"_2025_final.pdf"),plot=bestmapimg[[sp[l]]][[1]],  
