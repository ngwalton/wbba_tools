...
# ebird data
#ebird <- read.delim("WBBA2_ATLASWI_plussensitivepluszerocount_EBDMar2023_GOOD_082823.txt", quote = "", as.is = TRUE)
...
chronplot <- function(comname, ebird, pal, cex.x.axis = 0.9, cex.y.axis = 0.3) {
  ...
  if (first_obs == last_obs) {
    labels <- first_obs  # Handles the case of a single observation date
  } else {
    ...
    # Remove month-start labels that are within 10 days of the first or last observation
    date_buffer <- 10
    labels <- labels[!(labels > first_obs & labels <= first_obs + days(date_buffer))]
    labels <- labels[!(labels < last_obs & labels >= last_obs - days(date_buffer))]
    
    # Add the original first and last observation dates
    labels <- unique(c(first_obs, last_obs, labels))
