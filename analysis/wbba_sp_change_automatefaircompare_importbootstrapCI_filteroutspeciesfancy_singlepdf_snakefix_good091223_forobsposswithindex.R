...
# THIS VERSION INCLUDES GABRIELS FANCIER EXTRACTION OF NON-STANDARD SPECIES

# THIS VERSION ASSUMES COLUMNS ARE STARTING AS snake_case
...
boots <- read.csv("obspossCIs20230914.csv")
...
### filter out C1 and C2, and blank
sp$ii<-sp$ii%>%filter(!BREEDING.CATEGORY%in%c("C1","C2",""))
sp$i<-sp$i%>%filter(!BREEDING.CATEGORY%in%c("C1","C2",""))
