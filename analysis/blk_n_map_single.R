...
mo <- c("January", "February", "March", "April", "May", "June", "July",
...
prep_sp <- function(sp_df, blk, taxa, mo, priority_only, priority_lvls) {
  ...
  # create a sf data.frame from "sp_df"
  wgs84 <- st_crs(4326)
  sp_df <- st_as_sf(sp_df, crs = wgs84, coords = c("LONGITUDE", "LATITUDE"))

  # transform projection to match blocks
  sp_df <- st_transform(sp_df, st_crs(blk))
...
make_map <- function(sp, species, cnty, block_in, pal, tax, clip_box, season_pal) {
  ...
  currentII <- currentII[! is.na(currentII$N), ]  # remove "missing" blocks from II
...
# note that prep_sp returns a data.table
...
# create a sf object from "pt_count"
pt_count <- st_as_sf(pt_count, crs = st_crs(blk),
...
# print an n records map for each species to 1 or more pdfs; note that this can
# be time consuming
