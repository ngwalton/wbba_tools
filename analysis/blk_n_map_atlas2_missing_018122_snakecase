...
# IN THIS VERSION: TWO maps per species, one each for year,
# and all blocks with no records ("missing").
...
sp <- read.delim("22EBDmergedwithSensitiveSpeciesandZeroCountRecords.txt", quote = "", as.is = TRUE)

# sample WBBA I from eBird
sp1 <- read.delim("ebd_US-WI_199501_200012_relJul-2022.txt", quote = "", as.is = TRUE)

# REMOVE SENSITIVE SPECIES FROM THIS VERSION BECAUSE WE'RE MAKING IT PUBLIC
sp <- subset(sp, sp$common_name !="Sharp-tailed Grouse")
sp <- subset(sp, sp$common_name !="King Rail")
sp <- subset(sp, sp$common_name !="Whooping Crane")
sp <- subset(sp, sp$common_name !="Piping Plover")
sp <- subset(sp, sp$common_name !="Northern Goshawk")
sp <- subset(sp, sp$common_name !="Barn Owl")
sp <- subset(sp, sp$common_name !="Northern Hawk Owl")
sp <- subset(sp, sp$common_name !="Great Gray Owl")
sp <- subset(sp, sp$common_name !="Long-eared Owl")
sp <- subset(sp, sp$common_name !="Gyrfalcon")
sp <- subset(sp, sp$common_name !="Loggerhead Shrike")
sp <- subset(sp, sp$common_name !="Kirtland's Warbler")

sp1 <- subset(sp1, sp$common_name !="Sharp-tailed Grouse")
sp1 <- subset(sp1, sp$common_name !="King Rail")
sp1 <- subset(sp1, sp$common_name !="Whooping Crane")
sp1 <- subset(sp1, sp$common_name !="Piping Plover")
sp1 <- subset(sp1, sp$common_name !="Northern Goshawk")
sp1 <- subset(sp1, sp$common_name !="Barn Owl")
sp1 <- subset(sp1, sp$common_name !="Northern Hawk Owl")
sp1 <- subset(sp1, sp$common_name !="Great Gray Owl")
sp1 <- subset(sp1, sp$common_name !="Long-eared Owl")
sp1 <- subset(sp1, sp$common_name !="Gyrfalcon")
sp1 <- subset(sp1, sp$common_name !="Loggerhead Shrike")
sp1 <- subset(sp1, sp$common_name !="Kirtland's Warbler")

# point count data
pt_count <- read_excel("WBBAPtCts_Final4Years_v1_03082022_forblocknmap.xlsx",
                       sheet = "WBBAPtCts_Final4Years_v1_011420")
...
prep_sp <- function(sp_df, blk, taxa, mo, priority_only, priority_lvls) {
  ...
  sp_df <- sp_df[sp_df$common_name != "Domestic goose sp. (Domestic type)", ]
  ...
  sp_df$breeding_code <- trimws(sp_df$breeding_code)

  # add date columns
  sp_df$DDDD <- ymd(sp_df$observation_date)
  ...
  period_levels <- c("Year")
  sp_df$period[indx] <- period_levels[1]
  sp_df$period[! indx] <- period_levels[1]
  ...
  sp_df <- sp_df[breeding_category %in% c("C2", "C3", "C4"), ]

  # remove records duplicated across shared checklists
  sp_g <- sp_df[group_identifier != ""]  # split records with group id set
  sp_df <- sp_df[group_identifier == ""]    # records without group id are not shared
  cols <- c("common_name", "subspecies_common_name", "group_identifier")
  sp_g <- sp_g[! duplicated(sp_g, by = cols)]
  sp_df <- rbind(sp_df, sp_g)

  # order taxonomically
  setorder(sp_df, taxonomic_order, global_unique_identifier)

  # sum across June/July and not June/July
  sp_mo <- sp_df[, .N, by = .(common_name, BLOCK_ID, period)]

  # sum across all months
  sp_df$period <- "Year"
  sp_df <- sp_df[, .N, by = .(common_name, BLOCK_ID, period)]
...
make_map <- function(sp, species, cnty, block_in, pal, tax, clip_box, season_pal) {
  ...
  current <- sp[common_name == species, ]
  ...
  blk_map <- bg_map +
    ...
    tm_layout(title = m_title, title.size = 1,
      outer.margins = c(.02, 0.005, .02, .02),
...
#  }
  ...
  out <- tmap_arrange(blk_map, widths = c(5, 1),
    outer.margins = NULL)
...
sp <- merge(sp, tax[, cols], by.x = "common_name",
...
pt_count$common <- alpha[match(pt_count$speciescode, alpha$SPEC), "common_name"]
...
if (print_map) {
  ...
  n <- length(unique(sp$common_name))
  ...
  if (split_fam) {
    ...
    for (j in seq_along(fam_vec)) {
      ...
      sp_vec <- unique(current_fam$common_name)
