...
out_shp <- "evidence_by_block_onepager2"

# name of output pdf file if printing maps
out_pdf <- "evidence_maps_onepage2r.pdf"
...
sp_in <- read.csv("grandfathered_cutdowntoobspossdates_V2.csv") 
...
if (print_map) {
  ...
  pal <- c("#00000001", "#00000001", "gray80", "#00000001", "#00000001", "#00000001") #muted ebird purple
  ...
  for (i in seq_along(sp_vec)) {
    ...
    out <-
      ...
      tm_polygons(species, title = "", border.col = NULL, border.alpha = 0,
                  lwd = 0, palette = pal, labels = c("", "", "Observed", "", "", "")) +
