...
sp_in <- read.csv("obsposs_species_withH_asobsposs_110923_good.csv") 
...
if (print_map) {
  ...
  pal <- c("black", "#7145AC", "#A3C8F0", "#00000001", "#00000001", "#00000001") #muted ebird purple
  ...
  for (i in seq_along(sp_vec)) {
    ...
    out <-
      ...
      tm_polygons(species, title = "Evidence", border.col = NULL, border.alpha = 0,
                  lwd = 0, palette = pal, labels = c("Confirmed", "Probable", "Obs/Poss", "", "", "")) +
