...
ebird <- read.delim("../data/chronplot_data/March2022dataset.txt", quote = "", as.is = TRUE)
...
maxcats <- group[group[, .I[which.max(highestcat)], by=common_name]$V1]

# should we keep this species?
maxcats$result <- ifelse(maxcats$highestcat < 2, FALSE, TRUE)

# join the boolean column to eBird table
lookup <- maxcats[, c("common_name","result")]
ebird <- join(ebird, lookup, by = "common_name")
...
ebird <- subset(ebird, common_name != "American Robin")
...
ebird <- subset(ebird, common_name != "Red-winged Blackbird")
ebird <- subset(ebird, common_name != "Song Sparrow")

# splits into individual csv files by species
d_ply(ebird, .(common_name),
      function(ebird) write.csv(ebird,
                                file=paste(ebird$common_name[[1]],".csv",sep="")))
...
template <- "interactive_chronplot_snakecase.Rmd"
