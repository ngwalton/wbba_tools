...
if (print_map) {
  ...
  pal <- c("#2D1C45", "#9C7DC5", "#D4C7E6", "#00000001", "#00000001", "#00000001") #muted ebird purple
  ...
  for (i in seq_along(sp_vec)) {
    pdf(file=paste0(sp_vec[i],".pdf"),onefile = F, width = 3.75, height = 3.9469)
    if (i == 1) {
    ...
    out <-
      #this prints invisible but sets the correct dimensions for the next EL layer
      tm_shape(ecoland) +
      tm_polygons(border.col = "#90EE90", lwd = 0, lty = "dashed", fill_alpha = 0, col_alpha = 0) +
      #this is the block layer
      tm_shape(block_map, is.master = TRUE) +
      tm_polygons(species, legend.show = FALSE, border.col = NULL, col_alpha = 0,
                  lwd = 0, palette = pal ) +
      #this is the county layer       
      tm_shape(cnty) +
            tm_polygons(border.col = "gray60", lwd = 0.25, fill_alpha = 0, col_alpha = 1,
                  legend.show = FALSE) +
      #this is the smoothed EL layer
      tm_shape(ecoland2) +
            tm_lines("gray80", lwd = 0.25, lty = "dashed") +
      tm_layout(frame = FALSE, outer.margins = 0.005, inner.margins = 0) 
