...
# This version only pulls out hybrid records
...
sp_in <- read.delim("WBBA2_ATLASWI_plussensitivepluszerocount_EBDMar2023_GOOD_082823.txt", quote = "", as.is = TRUE)
...
# limit to hybrids (using taxon concept instead of common name because maybe the parentheses screwing something up)
sp_in <- sp_in[grepl("avibase-43C9473C|avibase-078BAB2B|avibase-6ECE73F4|avibase-362AF07B|avibase-83C5E013", sp_in$TAXON.CONCEPT.ID), ]
...
# only one value for BWWA x GWWA
sp_in$COMMON.NAME[sp_in$COMMON.NAME == "Brewster's Warbler (hybrid)"] <- "Golden-winged x Blue-winged Warbler (hybrid)"
sp_in$COMMON.NAME[sp_in$COMMON.NAME == "Lawrence's Warbler (hybrid)"] <- "Golden-winged x Blue-winged Warbler (hybrid)"
...
sp_in$SPEC[sp_in$COMMON.NAME == "Golden-winged x Blue-winged Warbler (hybrid)"] <- "GWBW" 
sp_in$SPEC[sp_in$COMMON.NAME == "Mallard x American Black Duck (hybrid)"] <- "MAAB" 
sp_in$SPEC[sp_in$COMMON.NAME == "Sandhill x Whooping Crane (hybrid)"] <- "SCWC" 
